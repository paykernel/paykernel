...
The shared kernel is [`@paykernel/example-checkout-kernel`](/examples/checkout-kernel). Five thin HTTP hosts map routes onto it. SDK packages used here are **`0.1.0-next.0` and are not on npm as a released 1.0**.
