...
export const D1_STORAGE_ADAPTER_MANIFEST: StorageAdapterManifest = {
  ...
  notes: [
    ...
    "createD1Executor / createD1PaymentStores / migrateD1Adapter default session to 'first-primary' when db.withSession exists; pass session: false to opt out (stale replica reads possible under read replication).",
    "Without sessions, stale replica reads are possible under D1 read replication — post-claim SELECTs used for claim classification can misclassify under replication.",
    "createD1Executor does not attach transaction() and never issues BEGIN IMMEDIATE on live D1 bindings. withTransaction fails closed (StoreUnsupportedFeatureError) when D1Executor.transaction is unavailable — no silent no-op multi-mutation atomicity; prefer single-statement claims or batch().",
