...
export const D1_STORAGE_ADAPTER_MANIFEST: StorageAdapterManifest = {
  ...
  notes: [
    ...
    "createD1PaymentStores / binding factories default session to 'first-primary' when db.withSession exists; pass session: false to opt out (stale replica reads possible under read replication).",
    ...
    "withTransaction fails closed (StoreUnsupportedFeatureError) when D1Executor.transaction is unavailable — no silent no-op multi-mutation atomicity; prefer single-statement claims or batch().",
