/**
 * Map D1 / storage errors into the testkit StoreErrorCode taxonomy.
 *
...
function sanitizeMessage(message: string, fallback: string): string {
  const cleaned = message
    ...
    // Cloudflare API tokens / account identifiers
...
/**
 * Heuristic mapping of D1 / SQLite / network / Workers errors.
 */
export function mapDriverError(err: unknown): StoreError {
  ...
  // Connection / unavailable / D1 internal
  if (
    code === "ECONNREFUSED" ||
    code === "ECONNRESET" ||
    code === "ENOTFOUND" ||
 ...
 * Used so store wrappers do not rewrite user-thrown Errors into StoreUnavailableError.
 */
