...
**Package:** `@paykernel/store-contracts`  
**Source:** [`contracts.ts`](../src/contracts.ts), [`adapter-manifest.ts`](../src/adapter-manifest.ts)  
**Conformance (testkit):** [`idempotency-conformance.ts`](../../testkit/src/storage/idempotency-conformance.ts), [`webhook-inbox-conformance.ts`](../../testkit/src/storage/webhook-inbox-conformance.ts), [`reconciliation-conformance.ts`](../../testkit/src/storage/reconciliation-conformance.ts)  
**Memory reference (NON_PRODUCTION, testkit):** [`memory-stores.ts`](../../testkit/src/memory/memory-stores.ts)  
**BC:** `@paykernel/testkit` re-exports this package.
...
**Where things live.** Lease-aware interfaces, `StoreError` subclasses, and `StorageAdapterManifest` are defined in **this package**. `@paykernel/testkit` re-exports them and hosts conformance suites + NON_PRODUCTION memory factories. Phase 10 **`@paykernel/webhooks`** owns the inbox **engine** and dual-owns a structurally compatible `WebhookInboxStore`. Phase 19 **`@paykernel/reconciliation`** owns reconciliation primitives and dual-owns a structurally compatible `ReconciliationStore`. Core **must not** depend on store-contracts, testkit, webhooks, or reconciliation. Core 0.x `IdempotencyStore` (get/set/reserve) is a **different** API — see [§12](#12-distinction-from-core-0x-idempotencystore).
...
**Phase 11 relational foundation (publishable):** shared SQL schemas, codecs, versioned migrations, and dialect-honest atomic claim templates live in public [`@paykernel/sql-foundation`](../../sql-foundation/README.md) (`packages/sql-foundation`). It is **not** a public ORM. The monorepo keeps a private thin re-export at [`internal/sql-store`](../../../internal/sql-store/README.md) (`@paykernel/internal-sql-store`, never published). Phase 9 contracts live in [`@paykernel/store-contracts`](../../store-contracts) (testkit re-exports for BC + conformance); foundation aligns field names by convention for Phase 12+ adapters. See [relational-foundation.md](../../sql-foundation/docs/relational-foundation.md) · [atomic-claims.md](../../sql-foundation/docs/atomic-claims.md).
...
**Source:** [`adapter-manifest.ts`](../src/adapter-manifest.ts) (re-exported from `@paykernel/testkit`)
...
`concurrency: true` is **same-isolate only**. It cannot mechanically fail a multi-host adapter that implements claim as non-atomic get-then-set. **Phase 11** lands the shared SQL foundation and multi-connection claim **intent/templates** in [`@paykernel/sql-foundation`](../../sql-foundation/docs/atomic-claims.md). **Phase 12 `@paykernel/store-postgres`** additionally proves multi-connection atomicity against real PostgreSQL (env-gated harness in the adapter package), for example:
...
| Shared SQL schemas / migrations / claim templates                                             | **Phase 11** — [`@paykernel/sql-foundation`](../../sql-foundation/README.md) (publishable)                                           |
| Production adapters other than postgres/redis/sqlite/turso/d1/do | Later phases (postgres Phase 12: [`@paykernel/store-postgres`](../../store-postgres/README.md); redis Phase 13 optional: [`@paykernel/store-redis`](../../store-redis/README.md); sqlite Phase 14 single-host: [`@paykernel/store-sqlite`](../../store-sqlite/README.md); turso Phase 15 multi-host remote: [`@paykernel/store-turso`](../../store-turso/README.md); D1 Phase 16 multi-host Workers: [`@paykernel/store-d1`](../../store-d1/README.md); DO Phase 17 multi-host partitioned: [`@paykernel/store-durable-objects`](../../store-durable-objects/README.md)) |
...
Phase 9 ends when contracts, fencing, errors, memory self-proof, manifests, and this documentation are in place — **not** when a durable database driver lands. Phase 10 adds the engine package on top of these store contracts without replacing conformance ownership. Phase 11 adds publishable `@paykernel/sql-foundation` (schemas, migrations, claim templates — not a general SQL API). Phase 12 implements production `@paykernel/store-postgres` on top of these contracts + sql-foundation templates. Phase 13 implements optional `@paykernel/store-redis` (Lua claims; configuration-dependent durability) without making Redis mandatory. Phase 14 implements single-host `@paykernel/store-sqlite` (BEGIN IMMEDIATE + sql-foundation sqlite templates; honest `single-host` manifest). Phase 15 implements multi-host remote `@paykernel/store-turso` (async UPSERT claims; honest multi-host remote; no untested `/sync`). Phase 16 implements multi-host Workers `@paykernel/store-d1` (async UPSERT claims on D1 binding; honest multi-host; sessions for RAW; not sqlite/turso/DO). Phase 17 implements multi-host partitioned `@paykernel/store-durable-objects` (sync in-object SQL UPSERT / `transactionSync`; deterministic sharding; strong RAW within partition; not D1/sqlite/turso; never one global DO).
...
- Phase 11 SQL foundation: [`../../sql-foundation/docs/relational-foundation.md`](../../sql-foundation/docs/relational-foundation.md)
