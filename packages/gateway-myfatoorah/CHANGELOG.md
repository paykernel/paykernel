...
- **MF-CREATE-REPLAY:** Outside KWT/SAU `Idempotency-Key` is omitted; a post-submit timeout / `afterProviderSubmit` / `indeterminate` on `createPayment` first looks up `CustomerReference = orderId` (or `myfatoorahCustomer.reference`) via `GetPaymentStatus` before replay to avoid double-charge — only **Paid** invoices are reused (`Pending` falls through because `GetPaymentStatus` has no `PaymentURL`). KWT/SAU still use header dedupe (250-minute cache). Provide `orderId` for replay safety outside KWT/SAU.
