...
- **MF-CREATE-REPLAY:** Outside KWT/SAU `Idempotency-Key` is omitted; a post-submit timeout / `afterProviderSubmit` / `indeterminate` on `createPayment` first looks up `CustomerReference = orderId` (fallback `idempotencyKey`) via `GetPaymentStatus` before replay to avoid double-charge. KWT/SAU still use header dedupe (250-minute cache).
