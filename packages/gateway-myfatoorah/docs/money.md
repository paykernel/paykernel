...
- **Webhooks (`PAYMENT_STATUS_CHANGED` / `REFUND_STATUS_CHANGED`):** **base** currency — `Data.Amount.ValueInBaseCurrency` + `BaseCurrency` preferred (fallback to display/pay then legacy `Value`); refund webhooks also base (MF-WEBHOOK-MONEY-DRIFT). `KD`/`SR` aliases are normalized to `KWD`/`SAR`.
