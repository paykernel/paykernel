...
- **`getPayment`:** **pay** when available — prefers `PaidCurrency` + `PaidCurrencyValue` (or `TransationValue` typo) from the last `SUCCESS`/`SUCCSS` transaction; if pay currency/value is missing or unparseable, falls back to `InvoiceValue` + `Currency` (**base** per https://docs.myfatoorah.com/docs/get-payment-status). Never mixes `InvoiceValue` (base) with `PaidCurrency` (pay) — MF-GETPAYMENT-BASE-MIX. `getPayment` is **refund-blind** — after a refund `GetPaymentStatus` still returns `Paid`; use `GetRefundStatus` / `REFUND_STATUS_CHANGED` (see `docs/status-mapping.md` I3). Fixtures use KWD base + SAR pay to lock this drift.
- **Webhooks (`PAYMENT_STATUS_CHANGED` / `REFUND_STATUS_CHANGED`):** **base** currency — `Data.Amount.ValueInBaseCurrency` + `BaseCurrency` preferred (fallback to display/pay then legacy `Value`); refund webhooks also base (MF-WEBHOOK-MONEY-DRIFT). A SAR checkout can webhook as KWD. Do **not** compare webhook `amount` to the create/getPayment pay amount for fulfillment — use `status === "paid"` / `isPaidOutcome` / `isMyFatoorahPaidTerminal` and `getPayment`. `KD`/`SR` and dotted variants (`K.D.`/`S.R.`/`K.D`/`S.R..`) are normalized to `KWD`/`SAR` via `normalizeMyFatoorahCurrency`.
- **`refundPayment` / `MakeRefund`:** **base** currency only (e.g. KWD), never display/pay — `InvoiceValue` is base per https://docs.myfatoorah.com/docs/get-payment-status. Sandbox (`live: false`) base is always `KWD`; live base is `BaseCurrency` from `GetRefundStatus` or, for first refund with empty history, inferred from `country` only when `live: true` (otherwise `KWD`) — MF-SANDBOX-BASE. Mismatch throws.

See also [Charges](./charges.md) and [Webhooks](./webhooks.md) for the pay-vs-base contract and [Refunds](./refunds.md) for the sandbox rule. Webhook mapper `webhookMoneyFromAmountRecord` prefers base → display → pay and handles commas.
