...
- **Idempotency:** `createPayment` requires a caller `idempotencyKey`, sent as the `Idempotency-Key` header **only in KWT/SAU** (omitted elsewhere). Outside KWT/SAU create looks up `CustomerReference = orderId` (or `myfatoorahCustomer.reference`) and reuses a **Paid** invoice only when amount+currency match — `Pending` is not reused; inquiry 5xx/429 is `indeterminate` (no second create). KWT/SAU skip that preflight. Header-not-supported validation retries once without the header; key conflicts do not. Refunds require the key (`ExternalIdentifier` + `GetRefundStatus` preflight + header only in KWT/SAU; headerless MakeRefund is `retry: false`). `SR`→`SAR` aliased. Any existing `ExternalIdentifier === idempotencyKey` refund is returned before `MakeRefund` (partial-replay safe)
