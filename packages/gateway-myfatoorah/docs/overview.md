...
- **Idempotency:** `createPayment` requires a caller `idempotencyKey`, sent as the `Idempotency-Key` header **only in KWT/SAU** (omitted elsewhere). Outside KWT/SAU a post-submit timeout / `afterProviderSubmit` / `indeterminate` first looks up `CustomerReference = orderId` (or `myfatoorahCustomer.reference`) via `GetPaymentStatus` to map an existing **Paid** invoice before replay — never double-charges; `Pending` is not reused (`GetPaymentStatus` has no `PaymentURL`). Outside KWT/SAU without `orderId`/`myfatoorahCustomer.reference` there is no dedupe — provide `orderId`. Refunds also require the key (`ExternalIdentifier` + `GetRefundStatus` preflight + header only in KWT/SAU). Header 2xx `IsSuccess:false` `ValidationErrors`/`FieldsErrors` string-body rejections are retried once without the header. `SR`→`SAR` aliased. Any existing `ExternalIdentifier === idempotencyKey` refund is returned before `MakeRefund` (partial-replay safe)
