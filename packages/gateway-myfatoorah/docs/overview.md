...
- **Idempotency:** `createPayment` requires a caller `idempotencyKey`, sent as the `Idempotency-Key` header **only in KWT/SAU** (omitted elsewhere). Outside KWT/SAU `orderId` or `myfatoorahCustomer.reference` is required; create looks up that `CustomerReference` and reuses a **Paid** invoice only when amount+currency match — `Pending` / amount mismatch / inquiry 5xx/429 / non-404 4xx / empty success `Data` are `indeterminate` with the existing **InvoiceId** when known (no second create). Official 2xx not-found `Message` (empty Data) may create. Unkeyed `/v3/payments` and first `MakeRefund` do not retry `429`. KWT/SAU skip that preflight. Header-not-supported validation retries once without the header; key conflicts do not. Refunds require the key (`ExternalIdentifier` + `GetRefundStatus` preflight + header only in KWT/SAU; headerless MakeRefund is `retry: false`). `SR`→`SAR` aliased. Any existing `ExternalIdentifier === idempotencyKey` refund is returned before `MakeRefund` (partial-replay safe)
