...
3. `remaining = InvoiceValue − (Refunded/Pending refunds)`. Canceled refunds do not count. Negative or unparseable remaining **fails closed** (even with explicit `amount`); empty refund list → `remaining = InvoiceValue` (no refunds yet). Amounts compared via minor units to avoid IEEE error.
4. Remaining `0` → map the nested refund whose `ExternalIdentifier` **exactly matches** this `idempotencyKey`; otherwise `InvalidRequestError` — never re-POST the invoice amount and never map a single unkeyed refund to an unrelated key. Lookup handles both `RefundStatusResult` and `Refunds` shapes.
