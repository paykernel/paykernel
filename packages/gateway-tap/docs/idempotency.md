...
- **capture / refund:** missing `idempotencyKey` throws `InvalidRequestError` before POST. These stay keyed: auto-retry (`withRetry`) is allowed because Tap deduplicates `reference.idempotent`. Capture of a `CAPTURED` authorize replays `POST /charges` with the same key. `VOID` authorizes are not capturable.
