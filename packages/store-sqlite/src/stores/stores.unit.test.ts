...
describe("NEW-PERF-8 deleteExpired default limit", () => {
  ...
  it("NEW-PERF-9: idempotency omit limit binds finite LIMIT (not unbounded DELETE)", async () => {
    ...
    const idempExec = recordingExecutor();
    await createSqliteIdempotencyStore({ executor: idempExec }).deleteExpired({
      before: "2099-01-01T00:00:00.000Z",
    });
    ...
    const explicit = recordingExecutor();
    await createSqliteIdempotencyStore({ executor: explicit }).deleteExpired({
      before: "2099-01-01T00:00:00.000Z",
      limit: 5,
    });
    expect(explicit.calls.find((c) => c.sql.includes("DELETE"))?.params[1]).toBe(5);
