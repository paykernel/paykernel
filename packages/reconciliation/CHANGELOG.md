...
- **PERF-7:** `claimDue` claims listed rows one-at-a-time (list is still not a fence). `processDue` claims immediately before each handler (NEW-RECON-2). `listDue` oversample when `maxInFlightByGateway` is set stays capped at 200.
