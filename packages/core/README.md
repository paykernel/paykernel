...
| **Fulfill on paid outcome / `status`, not `success`**  | `success: true` means API call OK (can be pending / 3DS). Prefer Phase 6 **`isPaidOutcome(result)`** or `outcome === 'succeeded'` with **`status === 'paid'`** (or `approved`). Never treat `indeterminate` as paid or as a definitive decline — reconcile. See [operation-results.md](./docs/operation-results.md). |
