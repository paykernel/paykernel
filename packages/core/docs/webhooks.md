...
**Merchants MUST dedupe by `event.id` (or an equivalent stable provider event
id) before fulfilling orders, capturing inventory, or sending goods.**

Providers redeliver the same event when your endpoint returns 5xx (including when
`onWebhookVerified` throws). Without an idempotency check on `event.id`, a
