...
| `CHECKOUT.ORDER.COMPLETED` | `paid` only when **nested** `purchase_units[].payments.captures` fully settle the order/auth total; **`partially_captured`** when nested captures are under-total, auth is `PARTIALLY_CAPTURED`, or a capture is non-final (`final_capture: false`) — dual-write **`payment.processing`**, not `payment.succeeded`. Auth-only (no capture) → **`authorized`** (aligns with `getPayment`). Bare `COMPLETED` without nested captures/authorizations — including a `supplementary_data.related_ids.capture_id` string with no nested captures — → **`processing`** (not `paid` / not `isPaidOutcome`) |
