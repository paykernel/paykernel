...
| **Webhook amounts** | `event.amount` and `event.currency` are present only when PayPal includes amount data. On multi-capture **order** webhooks, `event.amount` is the **sum of still-held captures** (aligned with `getPayment`; excludes fully REFUNDED face amounts), not the last capture slice. `CHECKOUT.PAYMENT-APPROVAL.REVERSED` does not include amount data in PayPal's documented payload, so those fields are undefined. |
