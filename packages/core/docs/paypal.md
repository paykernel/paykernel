...
| `PAYMENT.CAPTURE.REFUNDED` | `refunded` or `partially_refunded` when the **capture** resource status is `REFUNDED` / `PARTIALLY_REFUNDED`. Refund-shaped payloads (`resource_type=refund`, typically `COMPLETED`) are this-op refunds, not capture settlement — fail-closed **`partially_refunded`** (do not default to full `refunded`) |
