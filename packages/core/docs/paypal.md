...
| **Final capture** | PayPal API default for `final_capture` is `false`. SDK product defaults: **no amount** (full remaining) → `true`; **amount set** (partial) → `false` unless `paypalFinalCapture === true`. When the capture is non-final (`final_capture` omitted, `undefined`, or `false`), result/webhook/getPayment status is **`partially_captured`**, not `paid`. Only `final_capture === true` can map COMPLETED to `paid`. |
