...
> **Important — fulfillment**: Never fulfill on `captureResult.success` alone. Prefer **`isPaidOutcome(captureResult)`** (Phase 6: `outcome === 'succeeded'` **and** paid-like status **`paid` only**). Checking `status === 'paid'` is also fine. **`partially_captured`** (sale/order or auth capture when response `final_capture` is not `true`) has `outcome: 'requires_action'` and **`isPaidOutcome` is false** — do not treat it as full settlement. Sale/order `capturePayment` never sends `final_capture` and **does not** infer paid from a request default; HTTP 200 `COMPLETED` with omitted `final_capture` is **`partially_captured`**. **Never ship on buyer approval alone** (`status: 'approved'`). PayPal can return HTTP 200 with `status: 'pending'` (echeck / review). For webhooks, only treat **`PAYMENT.CAPTURE.COMPLETED`** as fulfillment when status maps to **`paid`** (resource `final_capture === true`); omitted/`undefined`/`false` completed captures map to `partially_captured` / `payment.processing`. Terminal failures return **`success: false`**.
>
...
> **Capture ID polling**: After `capturePayment`, `result.gatewayId` is the **capture ID**. Looking that ID up via `getPayment` hits `/v2/payments/captures/{id}` when the order path 404s. If the capture is non-final (`final_capture` is omitted, `undefined`, or `false`), status is **`partially_captured`** (not `paid`) and **`isPaidOutcome` is false** — same rule as `capturePayment` and webhooks. Multi-capture order lookups **omit** `captureId` unless exactly one refundable capture remains (never dual-write the latest id as a full-order refund target). They **aggregate** still-held capture amounts and demote status when auth is still `PARTIALLY_CAPTURED`, when `final_capture` is not `true`, when order/auth totals are missing/unparsable, or when held totals are under the order/auth amount. Capture-resource GET after `PARTIALLY_REFUNDED`/`REFUNDED`/`REVERSED` omits face amount unless net remaining can be proven from `seller_receivable_breakdown.total_refunded_amount` (full refund/reversal never publishes original face as still-held). Authorization GET **omits** `related_ids.capture_id` (siblings cannot be proven; auth id is not refundable). **Bare order `COMPLETED` without `payments.captures` / authorizations is `processing` (not `paid`)** — fail-closed so partial/missing payments payloads cannot fulfill via `isPaidOutcome`. Auth-only completed orders map to **`authorized`** (same as webhooks).
...
| **Webhook payload hash** | `event.payloadHash` is a compact identity (`id` / `event_type` as `type` / `create_time` / `resource.id`) — not a hash of the full resource tree. Inbox claim must use this digest; re-hashing `rawPayload` will not match. |
...
| **Status lookup IDs** | `getPayment()` and `getPaymentStatus()` accept PayPal order IDs, capture IDs, and authorization IDs, so the `gatewayId` returned from create, authorize, or capture can be checked later. Capture-resource lookup applies the same **`final_capture !== true` → `partially_captured`** demotion as capture/webhook paths (do not fulfill remaining auth on re-poll). Authorization lookup **omits** `related_ids.capture_id` unless siblings can be proven. |
...
| **Final capture** | PayPal API default for `final_capture` is `false`. SDK product defaults apply to **authorization** captures only: **no amount** (full remaining) → request `true`; **amount set** (partial) → request `false` unless `paypalFinalCapture === true`. Sale/order `capturePayment` does **not** send `final_capture` and does **not** treat omitted echo as paid. When the capture is non-final (`final_capture` omitted, `undefined`, or `false`), result/webhook/getPayment status is **`partially_captured`**, not `paid`. Only response `final_capture === true` can map COMPLETED to `paid`. |
...
|------------|---------------|
| `PAYMENT.CAPTURE.COMPLETED` | `paid` when `final_capture === true` (**preferred fulfillment signal**); **`partially_captured`** when resource `final_capture` is omitted, `undefined`, or `false` (PayPal API default is `false`; stable dual-write **`payment.processing`**, not `capture.completed`) |
| `PAYMENT.CAPTURE.DENIED` | `failed` |
| `PAYMENT.CAPTURE.DECLINED` | `failed` |
| `PAYMENT.CAPTURE.PENDING` | `pending` |
| `PAYMENT.CAPTURE.REFUNDED` | `refunded` or `partially_refunded` when the **capture** resource status is `REFUNDED` / `PARTIALLY_REFUNDED`. Refund-shaped payloads (`resource_type=refund`, typically `COMPLETED`) are this-op refunds, not capture settlement — fail-closed **`partially_refunded`** (do not default to full `refunded`). Mapper + rematch: proven `refunded` dual-writes **`refund.completed`**; missing / `partially_refunded` dual-writes **`refund.pending`**. |
| `PAYMENT.CAPTURE.REVERSED` | `reversed` |
| `CHECKOUT.ORDER.APPROVED` | `approved` (stable type `payment.processing` — **not** paid; do not fulfill) |
| `CHECKOUT.ORDER.COMPLETED` | `paid` only when **nested** `purchase_units[].payments.captures` fully settle the order/auth total **and** the preferred capture has `final_capture === true`; **`partially_captured`** when nested captures are under-total, order/auth totals are missing/unparsable, auth is `PARTIALLY_CAPTURED`, or a capture is non-final (`final_capture` omitted/`undefined`/`false`) — dual-write **`payment.processing`**, not `payment.succeeded`. Auth-only (no capture) → **`authorized`** (aligns with `getPayment`). Bare `COMPLETED` without nested captures/authorizations — including a `supplementary_data.related_ids.capture_id` string with no nested captures — → **`processing`** (not `paid` / not `isPaidOutcome`) |
...
| `PAYMENT.REFUND.COMPLETED` | **`refund_completed`** (this refund op finished; not proven full capture refund — fail-closed vs overstated `refunded`; stable dual-write **`refund.pending`**, not `refund.completed` — type-only handlers must not book a completed capture refund from this-op). Use `PAYMENT.CAPTURE.REFUNDED` capture-resource status / `getPayment` for aggregate completeness |
...
> `final_capture` omitted, `undefined`, or `false` normalize to **`partially_captured`** and dual-write stable
