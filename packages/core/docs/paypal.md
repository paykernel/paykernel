...
| **Multiple captures** | `getPayment` **and** multi-capture order webhooks (`CHECKOUT.ORDER.COMPLETED`) keep **latest** capture id (for refunds) by `update_time`/`create_time`, but **`amount` / `capturedAmount` / `event.amount` sum successful captures** (not last-slice only). Status is **`partially_captured`** when authorization is `PARTIALLY_CAPTURED`, when `final_capture` is false on the capture, or when capture totals are strictly less than order/auth amount — never prefer a COMPLETED slice over an open partial auth. |
