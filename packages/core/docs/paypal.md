...
| `CHECKOUT.ORDER.COMPLETED` | `paid` only when capture(s) fully settle the order/auth total; **`partially_captured`** when nested captures are under-total, auth is `PARTIALLY_CAPTURED`, or a capture is non-final (`final_capture: false`) — dual-write **`payment.processing`**, not `payment.succeeded`. Auth-only (no capture) → **`authorized`** (aligns with `getPayment`). Bare `COMPLETED` without captures/authorizations → **`processing`** (not `paid` / not `isPaidOutcome`) |
