...
After a successful capture, if Stripe returns `status: succeeded` and `amount_received` is finite and less than the authorized `amount`, the normalized status is `partially_captured` (same rule as `getPayment` / succeeded PaymentIntent webhooks).
...
2. **Partial capture**: if not refunded and `amount_received < amount`, status is `partially_captured`.
3. Otherwise the Stripe PaymentIntent status is mapped (`succeeded` → `paid`, `requires_capture` → `authorized`, etc.). Unmapped PaymentIntent statuses fail closed as `failed` (with a logger warning) so unknown states are not treated as pending fulfillment.

**Amount** prefers captured/settled money when available: for succeeded intents, `amount_received` is used when present and finite; otherwise `amount`. So `payment.amount` reflects the captured total after partial capture, not necessarily the original authorization.
...
`checkout.session.completed` with `payment_status: 'no_payment_required'` is normalized as `setup_completed` **only** when `mode === 'setup'` **or** a `setup_intent` is present. Payment-mode sessions that complete with `no_payment_required` (for example $0 free orders) stay `pending` rather than `setup_completed`.
