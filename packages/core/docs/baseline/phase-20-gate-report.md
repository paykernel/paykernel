# Phase 20 adversarial gate report (baseline)

**Date (UTC):** 2026-08-04  
**Gate kind:** Final adversarial re-gate (fail-closed)  
**Scope:** `@paykernel/opentelemetry` — portable metrics, spans, redacting telemetry glue, optional OTEL bridge; core `OperationContext` + `createRedactingTelemetrySink`  
**Reviewer stance:** fail-closed (missing evidence = blocking)  
**Verdict:** **PASS**

> Primary package: [`packages/observability`](../../../observability/)  
> Package report twin: [`packages/observability/docs/phase-20-gate-report.md`](../../../observability/docs/phase-20-gate-report.md)  
> Docs: [overview](../../../observability/docs/overview.md) · [operation-context](../../../observability/docs/operation-context.md) · [metrics](../../../observability/docs/metrics.md) · [instrumentation](../../../observability/docs/instrumentation.md) · [redaction](../../../observability/docs/redaction.md) · [opentelemetry](../../../observability/docs/opentelemetry.md) · [core telemetry](../telemetry.md)
...
| -- | --------- | ------- | -------- |
| **A1** | Provider request IDs support operational debugging | **PASS** | `OperationContext.providerRequestId` in `packages/core/src/runtime/operation-context.ts`; allow-listed in `SAFE_KEY_ALLOWLIST` (`logger.ts` → `providerrequestid`); not redacted by `createRedactingTelemetrySink`. Tests: `operation-context.test.ts` (round-trip through redacting sink), `packages/observability/src/redaction.test.ts` (A1), `instrumentation.test.ts` (emit keeps `req_visible`). Docs: `docs/telemetry.md`, observability `operation-context.md` / README. |
| **A2** | Core has no mandatory OpenTelemetry dependency | **PASS** | `packages/core/package.json`: deps = `{ zod }` only; no `@opentelemetry/*` peer. No static OTEL import under `packages/core/src`. Observability optional bridge: duck-typed `createOpenTelemetryBridge(injectedApi)` — **no** `from "@opentelemetry/api"` in production sources (JSDoc example only). `@opentelemetry/api` is **optional** peer of observability only. Root import works without OTEL (`public-api.test.ts` A2). Boundaries rules forbid core OTEL deps. |
| **A3** | Sensitive values are never emitted by default | **PASS** | `createRedactingTelemetrySink` wraps sink via core `redact()` (`gateways/gateway-context.ts`). Observability re-exports same helper; `redactTelemetryData` uses core `redact` (no reimplemented policy). `withPaymentOperation` / `recordPaymentOperation` always wrap telemetry through redacting sink; do not attach `error.message`. Tests: core `operation-context.test.ts` secrets → `[REDACTED]`; observability `redaction.test.ts` (A3); instrumentation scrub path. Same allow-list as logs. |
...
| ---- | -------- | -------- | ------- |
| **20.1** OperationContext fields | operation ID, gateway, operation type, tenant/namespace, internal ref, provider object ID, provider request ID, attempt, duration, normalized outcome, retry/recon flags, inbox event key | Exact fields on `OperationContext` / builders in `src/runtime/operation-context.ts`; re-exported from observability | **PASS** |
| **20.2** Span names | `payment.create` / `.capture` / `.refund` / `.void` / `.webhook.verify` / `.webhook.claim` / `.webhook.process` / `.reconcile` / `.store.claim` | `PAYMENT_SPAN_NAMES` in `packages/observability/src/spans.ts`; tests exact set | **PASS** |
| **20.3** Metrics instruments | outcomes, provider latency, rate limits, retries, webhook duplicates, payload conflicts, handler failures, expired/reclaimed leases, recon drift, indeterminate, adapter latency/errors | 13 instruments on `PaymentMetrics` (`metrics.ts` / `PAYMENT_METRICS_KEYS`); `METRIC_NAMES` under `payments.*` | **PASS** |
| **20.4** Redaction | same model as logs | `createRedactingTelemetrySink` → `redact()`; observability re-exports core; tests secrets + keep `providerRequestId` | **PASS** |
...
| Docs present | observability docs + core `telemetry.md` | **PASS** |
| No Phase 21 routing | no `createPaymentRouter` / route() API in packages | **PASS** |
| No production node:/bun: in observability src | portability.test.ts + independent scan | **PASS** |
