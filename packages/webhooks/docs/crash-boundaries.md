...
- `ackAfterClaim: true`: returns `scheduled_for_retry { reason: "parked" }` after durable claim/release **without** running the handler **only if** `workerGuaranteed: true` **and** `store.fail({ restoreAttempt: true })` succeeds. Requires a materializable `envelope` / `event` (else retryable `handler_failed` before claim — WEBHOOKS-2, not `invalid_webhook`). Without `workerGuaranteed`, the engine refuses parked (constructor throw or retryable `handler_failed`) so hosts cannot 200 with no worker (I6). Parking claim does not consume `maxAttempts` (`restoreAttempt`). Crash after that ACK is OK only if a **worker** runs `processRetryable`. ACK without a worker = lost work. Park `lease_lost` → `already_processing` / retryable `handler_failed` (never `parked`).
