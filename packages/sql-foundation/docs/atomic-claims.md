...
| `decideWebhookClaim`        | Webhook inbox  | No row → insert claimed; pending-when-due (`availableAt <= now`) or expired/idle lease → update (idle hash mismatch **supersedes** unless `ifMatchPayloadHash` misses); pending + same hash + future `availableAt` → `not_available`; expired lease reclaim allowed even if `availableAt` future; active lease + hash mismatch → `payload_hash_conflict`; `ifMatchPayloadHash` miss → `payload_hash_conflict` (no rewrite, S19); else completed / in_progress / duplicate_failed |
...
3. **Hash mismatch (NEW-SQL-1 / WEBHOOKS-3):** without `ifMatchPayloadHash`, `payload_hash_conflict` is **only** for an **active** lease. Idle pending / expired claimed rows **supersede** the stored hash (`decideWebhookClaim`) so a hash-source mistake (raw string vs object) does not permanently stick a paid redrive. **S19-WH-HASH-TOCTOU:** when `ifMatchPayloadHash` is set and the stored hash differs, return `payload_hash_conflict` **without rewriting** (retry workers must not roll a newer idle body backwards). Omit `ifMatchPayloadHash` on first-delivery so idle mismatch still supersedes.
