...
| `decideWebhookClaim`        | Webhook inbox  | No row → insert claimed; matching payload_hash + pending/expired → update; else completed / in_progress / payload_hash_conflict / duplicate_failed                |
...
1. Insert if absent as `claimed`, or reclaim `pending` / expired lease.
2. `payload_hash` must match on conflict; mismatch is conflict (not silent overwrite).
3. Do not reclaim terminal `completed` / `failed` / `dead_letter` via this template path.
4. Increment fencing fields; bind all user values.
...
Complete/fail templates (`idempotencyCompleteTemplates`, `webhookCompleteTemplates`, `webhookFailTemplates`) fence with `WHERE lease_token = … AND status = … AND lease_expires_at > now` so stale tokens update **zero** rows.
...
- `src/reference/bun-sqlite-store.ts` — NON-PRODUCTION bun:sqlite reference (not on main export)
